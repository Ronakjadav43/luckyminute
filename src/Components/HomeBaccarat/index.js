...
import "./HomeBaccarat.scss";

const HomeBaccarat = (props) => {
  ...
  const gameReducer = useSelector((state) => ({
    ...
    isBaccaratChipsActive: state.gameReducer.isBaccaratChipsActive,
    ...
    baccaratCardLength: state.gameReducer.baccaratCardLength,
    baccaratAppliedChips: state.gameReducer.baccaratAppliedChips,
  }));
  ...
  useEffect(() => {
    if (gameReducer.sessions !== null) {
      ...
      props.actions.baccaratGame.getBetsBySession(
  ...
  useEffect(() => {
    if (gameReducer.baccaratResult) {
      setResult(gameReducer.baccaratResult);
    }
  }, [gameReducer.baccaratResult]);
      ...
      if (activeStep) {
        const timer = setInterval(() => {
          if (activeStep.id === 1 && newMinute === 0) {
            props.actions.baccaratGame.clearBetSelection();
            setMessage("No More Bets");
            setStepId(9);
          }
          if (activeStep.id === 4 && newMinute === 2) {
            props.actions.baccaratGame.showWinnigArea();
            props.actions.userAction.getUserBalanceAndMessages();
          }
          setMinutes(newMinute);
        }, 1000);

        if (activeStep.id === 3 && minutes === 24) {
          if (gameReducer.baccaratCardLength === 6) {
            setMinutes(21);
          } else if (gameReducer.baccaratCardLength === 5) {
            setMinutes(18);
          } else {
            setMinutes(13);
          }
          ...
          switch (activeStep.id) {
            case 1:
              if (nextActiveStep) {
                ...
                const bets = gameReducer.baccarat.filter(
                  (baccarat) => baccarat.chip !== null
                );
                if (bets.length > 0) {
                  props.actions.baccaratGame.getBetsBySession(
                    gameReducer.userDetail.id,
            ...
            case 2:
              if (nextActiveStep) {
                props.actions.baccaratGame.getBaccaratResult(sessions);
                nextActiveStep.isActiveStep = true;
                nextActiveStep.time = 25;
                setPage([...page]);
                setMessage(nextActiveStep.message);
                setStepId(nextActiveStep.id);
              }
            ...
            case 3:
              if (nextActiveStep) {
                ...
                dispatch({
                  type: SHOW_BACCARAT_RESULT_AREA,
                });
                nextActiveStep.isActiveStep = true;
            ...
            case 7:
              if (nextActiveStep) baccaratStart();
              break;
  ...
  const handleChip = (selectedChip) => {
    if (selectedChip <= amount) {
      props.actions.baccaratGame.baccaratApplyChip(selectedChip);
    } else {
      dispatch({
        type: ERROR,
  ...
  return (
    <div>
      ...
      <div className="homebaccarat">
        ...
        <Footer
          message={message}
          isChipsActive={gameReducer.isBaccaratChipsActive}
        ...
        {stepId === 1 && gameReducer.isBaccaratChipsActive ? (
...
export default connect(null, mapDispatchToProps)(HomeBaccarat);
