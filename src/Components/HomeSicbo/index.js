...
import "./HomeSicBo.scss";

const HomeSicBo = (props) => {
  ...
  const gameReducer = useSelector((state) => ({
    ...
    isSicboChipsActive: state.gameReducer.isSicboChipsActive,
    ...
    sicboAppliedChips: state.gameReducer.sicboAppliedChips,
  }));
  ...
  useEffect(() => {
    if (gameReducer.sessions !== null) {
      ...
      props.actions.sicboGame.getBetsBySession(
  ...
  useEffect(() => {
    let sum = 0;
    if (gameReducer.sicboResult) {
      gameReducer.sicboResult.map((result) => (sum += result));
      setTotal(sum);
      setResult(
      ...
      if (activeStep) {
        const timer = setInterval(() => {
          if (activeStep.id === 1 && newMinute === 0) {
            props.actions.sicboGame.clearBetSelection();
            setMessage("No More Bets");
            setStepId(9);
          }
          if (activeStep.id === 4 && newMinute === 2) {
            props.actions.sicboGame.showWinnigArea();
            props.actions.userAction.getUserBalanceAndMessages();
          }
          ...
          switch (activeStep.id) {
            case 1:
              if (nextActiveStep) {
                ...
                const bets = gameReducer.sicbo.filter(
                  (sicbo) => sicbo.chip !== null
                );
                if (bets.length > 0) {
                  props.actions.sicboGame.getBetsBySession(
                    gameReducer.userDetail.id,
            ...
            case 2:
              if (nextActiveStep) {
                props.actions.sicboGame.getSicboResult(sessions);
                nextActiveStep.isActiveStep = true;
  ...
  const handleChip = (selectedChip) => {
    if (selectedChip <= amount) {
      props.actions.sicboGame.sicboApplyChip(selectedChip);
    } else {
      dispatch({
        type: ERROR,
  ...
  return (
    <div>
      <Notification />
      <div className="homesicbo">
        <Header name="骰宝 Sic Bo" />
        <div className="game_details">
          <GameDetails header={"Game ID"} value={"0123456789"} />
          <GameDetails header={"Balance"} value={amount} />
          <GameDetails header={"Bet"} value={gameReducer.sicboAppliedChips} />
        </div>
        ...
        <Footer
          message={message}
          isChipsActive={gameReducer.isSicboChipsActive}
        ...
        {stepId === 1 && gameReducer.isSicboChipsActive ? (
...
export default connect(null, mapDispatchToProps)(HomeSicBo);
